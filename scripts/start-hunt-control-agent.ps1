...
$AgentJar = & (Join-Path $ProjectRoot 'scripts\build-hunt-control-agent.ps1') -JdkHome $JdkHome
...
$Log = Join-Path $ProjectRoot 'captures\hunt-control-agent.log'
...
Write-Output "Hunt control agent attached pid=$ProcessId port=$Port"
