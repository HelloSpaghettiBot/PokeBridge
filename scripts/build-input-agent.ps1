...
$SourceRoot = Join-Path $ProjectRoot 'tools\input-agent\src'
$BuildRoot = Join-Path $ProjectRoot 'analysis\input-agent-build'
$ClassRoot = Join-Path $BuildRoot 'classes'
$AgentJar = Join-Path $BuildRoot ("input-agent-{0}.jar" -f (Get-Date -Format 'yyyyMMddHHmmssfff'))
...
& (Join-Path $JdkHome 'bin\jar.exe') --create --file $AgentJar `
  --manifest (Join-Path $ProjectRoot 'tools\input-agent\MANIFEST.MF') -C $ClassRoot .
