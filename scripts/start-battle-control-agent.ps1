...
$AgentJar = & (Join-Path $ProjectRoot 'scripts\build-battle-control-agent.ps1') -JdkHome $JdkHome
...
$Log = Join-Path $ProjectRoot 'captures\battle-control-agent.log'
...
Write-Output "Battle control agent attached pid=$ProcessId port=$Port"
