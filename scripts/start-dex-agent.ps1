...
$AgentJar = & (Join-Path $ProjectRoot 'scripts\build-dex-agent.ps1') -JdkHome $JdkHome
...
$Log = Join-Path $ProjectRoot 'captures\dex-location-agent.log'
...
Write-Output "Dex location agent attached pid=$ProcessId port=$Port"
